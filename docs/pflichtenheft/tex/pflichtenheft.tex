...
$\lambda$-Terme aus dem Text-Editor oder der Schrittliste können als LaTeX-Snippet in die Clibboard kopiert werden. Bis zu einer gewissen Größe ist es auch möglich, den kompletten Rechenweg mit zu exportieren.
...
\functionality{E-Mail Link}{fnc:export-email}
\fulfills{crt:share}
...
\teststep{ Im Editor steht der $\lambda$-Term ($\lambda$x. f x x) (($\lambda$y. y)z). Die Schrittanzahl für die zu gehenden Steps ist auf 1 eingestellt.}
{Der Benutzer drückt auf den \enquote{Step}-Button und drückt auf ($\lambda$y. y) um diesen Redex als nächstes zu reduzieren.}
{Das Programm wählt diesen Redex für die $\beta$-Reduktion und gibt als nächsten Step \enquote{($\lambda$x. f x x) z } aus.}
...
\teststep {Der Benutzer hat einen $\lambda$-Term eingegeben. Der $\lambda$-Term sollte mehr als 100 Zwischenschritte haben. Der Benutzer hat den $\lambda$-Term auswerten lassen. Lediglich das Ergebnis wurde ausgegeben.}
...
\teststep {Nach dem vorigen Test wurde noch 5 mal auf den \enquote {3-Punkte}-Button gedrückt, es sind also 90 Zwischenergebnisse sichtbar.}
...
\teststep {Der Benutzer hat einen $\lambda$-Term eingegeben und diesen Auswerten lassen. Dieser $\lambda$-Term sollte länger als 50 Zwischenschritte beinhalten. Er stellt die Anzahl der auszugebenden Zwischenergebnisse auf 50 um.}
...
\includegraphics[width=\textwidth]{../misc/GUI-Design/tutorials/Tutorial_Editor_Section.png}
\newline
\newline
\textbf{(1) Eingabefeld}
...
\includegraphics[width=\textwidth]{../misc/GUI-Design/tutorials/Tutorial_Sidebar_Section.png}
\newline
\newline
\textbf{(1) Schritte}
...
\includegraphics[width=\textwidth]{../misc/GUI-Design/tutorials/Tutorial_Top_Menu_Section.png}
\newline
\newline
...
\includegraphics[width=\textwidth]{../misc/GUI-Design/tutorials/Tutorial_Popups_Section.png}
\newline
\newline
\textbf{(1) Syntax Fehler}
\newline Dieses Pop-Up erscheint, sollte sich in der Eingabe ein Syntax Fehler befinden.

\textbf{(2) Schleifenerkennung}
\newline Dieses Pop-Up erscheint, wenn das Programm eine Endlosschleife erkennt.
