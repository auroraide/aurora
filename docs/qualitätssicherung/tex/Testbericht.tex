...
\section{Testfallüberdeckung per Package}
	...
	Dem Team ist es nicht gelungen Coverage auf ein paar Methoden anzuwenden, weil sie Fälle abfangen, die zu dem jetzigen Zeitpunkt nicht auftreten können.
	...
	Dieser zurzeit toter Code hätte für eine besser Coverage gelöscht werden können, es wurde sich aber dagegen entschieden, da die Möglichkeit besteht, 
	dass das Programm geforked wird und neue Tokens im Enum definiert wurden.
	Ein zweites Problem sind die Visitoren, die alle Methoden überschreiben müssen.
	
	Im HighlghtableLambdaExpression werden alle gebundenen Variablen in Freie Variablen geändert.
	Dennoch müssen alle Visitoren die Methode accept(BoundedVariable bvar) obwohl es unmöglich ist, dass diese Methode aufrufbar ist.
