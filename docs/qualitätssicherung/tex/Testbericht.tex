...
\subsection{Anzahl von dargestellten Schritten auswählen (M6)}
	In Funktionaler Anforderung \textbf{5.1} wurde für dieses Musskriterium ein "3-Punkte-Button"
	vorgestellt.
	Alle Funktionalität des "3-Punkte-Button" wurde in den Step-Button integriert, da die zwei Buttons nahezu identisch waren. 

\subsection{Pretty Print(K6)}
	Wie bereits im Entwurfsdokument beschrieben ist kein Pretty Print implementiert.
	
\subsection{Standardbibliothek (K7)}
...
\subsection{Schleifenerkennung (K19)}
...
\subsection{Funktionennamen (N18)}
