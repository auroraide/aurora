...
Im Folgenden ist das Ergebnis unserer Paketeverteilung.
\begin{description}
...
Diese Reihenfolge wurde gewählt, damit möglichst alles gleich testbar war und deshalb wurden zuerst die Abhängigkeiten implementiert.
    ...
    \item [Younis]\hfill \\
	\begin{enumerate}
		\item Implementierung von Lexer und Parser
		\item Implementierung von HighligtedLambdaExpression
		\item Implementierung der Simplifier Klassen (aurora.backend.simplifier.*)
	\end{enumerate}
    \item [Randy]\hfill \\
\end{description}
...
\subsection{Änderungen am Implementierungsplan}
