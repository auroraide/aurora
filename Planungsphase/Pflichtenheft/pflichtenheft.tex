...
\criterium{Input editieren}{crt:not-editable}
...
\criteriumOptional{Pause-Button}{crt:pausebutton}
...
\criteriumOptional{Nacht Modus}{crt:night-mode}
...
\subsection{Software}
	\begin{itemize}
		\item Es wird ein Windows, Linus oder MacOS OS benötigt.
		\item Es wird ein gängiger Webbrowser benötigt, der HTML5, CSS3 und JavaScript unterstützen kann: 
			\begin{itemize}
...
\subsection{Hardware}
	\begin{itemize}
		\item Es wird ein Rechner benötigt, der Internetverbindung erstellen kann.
		\item Es wird ein Rechner mit ausreichender Leistung benötigt. 
	\end{itemize}
\subsection{Orgware}
	\begin{itemize}
...
\functionality{Kommentare hinzufügen}{fnc:commentyourcode}\fulfills{crt:comments}
...
\functionality{Highlightning von Lambdas und Punkten}{fnc:highlightstatic}
\fulfills{crt:syntax-highlighting}
Lambda Symbole (\enquote{$\lambda$}) und Punkte (\enquote{.}) werden zur besseren Übersicht fett hervorgehoben.
\textcolor{green}{Sicher, dass wir fett und nicht farbig wollen?}

\functionality{Highlighting von Funktsionsnamen und Klammern}{fnc:highonbrackets}
\fulfills{crt:syntax-highlighting}
...
Wenn der Benutzer über eine Klammer hovert, werden beide Klammern des Paares farbig hervorgehoben.
...
\functionality{Highlighting des zuletzt ausgewerteten Redex}{fnc:highlight-prev}
\fulfills{crt:syntax-highlighting-premium}
...
\functionality{Ausführen}{fnc:run-input}
\fulfills{crt:evaluate-term}
...
\functionality{Stop-Button}{fnc:stop-exec} 
\fulfills{crt:stop-feature}
...
\functionality{Pause-Button} {fnc:pausebut} 
\fulfills{crt:pausebutton}
...
\functionality{Benutzerbibliothek}{fnc:user-vs-std}
...
\functionality{Entfernen aus der Benutzerbibliothek}{fnx:userlib-remove}
\fulfills{crt:user-library}
...
\functionality{Auswertungsstrategien}{fnc:eval-order-options}
...
\functionality{Auswahl der Auswertungsstrategie}{fnc:eval-order}
\fulfills{crt:opt-eval-strategy}
...
\functionality{Korrekte Darstellung in Google Chrome}{fnc:chrome-design}
...
\functionality{Nacht Modus}{fnc:night-mode}
...
\functionality{Mobile Darstellung ist im begrenzten Modus möglich}{fnc:mobile}
...
\functionality{Eingabe wird geparsed}{fnc:pars}
\textcolor{green}{HIER FEHLT BESCHREIBUNG}

\functionality{Ermittlung der eingestellten Auswertungsstrategie}{fnc:checkorder}
...
\functionality{Eine $\beta$-Reduktion}{fnc:beta}
...
\functionality{Folgende $\beta$-Reduktionen}{fnc:morebeta}
Solange $\beta$-Reduktionen möglich sind führt das Programm sie aus.

\functionality{Ausgabefeld}{fnc:result}
\fulfills{crt:evaluate-term}
...
\functionality{Offensichtliche Schleifenerkennung}{fnc:loop-test}
...
\functionality{Nach neuer Tab}{fnc:to-new-tab}
...
\functionality{LaTeX export}{fnc:latexshare}
\fulfills{crt:latex-export}
...
\functionality{Tutorial zur Web App}{fnc:tutorial}
...
\functionality{Session und Benutzer Bibliothek Speichern}{fnc:savesession}
\textcolor{green}{HIER FEHLT BESCHREIBUNG}
...
\functionality{$\beta$-Reduktionen angezeigen}{fnc:stepbystep}
...
\functionality{De Bruijin Indizes}{fnc:debruijn-output}\fulfills{crt:debruijn-output}
...
\tests{fnc:chrome-design}\tests{fnc:input}\tests{fnc:ln}\tests{fnc:mobile}
...
\test{Auswertung mit einem Schritt}{tst:single-step-eval}
\tests{fnc:input}
\tests{fnc:ln}
\tests{fnc:editor}
\tests{fnc:bracket}
\tests{fnc:highlightstatic}
\tests{fnc:highlight-next}
\tests{fnc:highlight-prev}
\tests{fnc:run-input}
\tests{fnc:eval-order-normal}
\tests{fnc:chrome-design}
\tests{fnc:output}
\tests{fnc:pars}
