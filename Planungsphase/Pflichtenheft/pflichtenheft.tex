...
Auf Wunsch kann ein einzelner Schritt ausgewertet werden.
...
\nonFunctionality{Eingabedefinition}{nfc:inpudefinition}
...
\teststep{Zustand vom vorherigen Schritt.}
{Der Benutzer füght eine neue Zeile am Ender der Eingabe hinzu mit $) \lambda x.x$ und drückt auf \enquote{Run}.}
...
%\tests{fnc:ln}
\tests{fnc:editor}
\tests{fnc:no-edit-while-running} % TODO update \tests
\tests{fnc:run}
...
Der \enquote{Run}-Knopf wird mit einem \enquote{Pause}-Knopf ersetzt.
Der \enquote{Step}-Knopf wird ausgegraut.}
...
Der \enquote{Pause}-Knopf wird mit einem \enquote{Continue}-Knopf ersetzt.
Der \enquote{Step}-Knopf ist nicht mehr ausgegraut.}
...
%{Das Programm gibt $$\lambda s.\lambda z.(\,s\,(\,s\,(\,s\,(s\,z)))$$ im Ausgabefeld aus. (Dieser Test gibt $4$ aus, wenn das Wunschkriterium Standardbibliothek vollständig implementiert ist).}
