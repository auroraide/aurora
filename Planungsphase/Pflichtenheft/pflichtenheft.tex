...
\test{Einstellungen}{tst:settings}
...
\teststep {Das Programm ist im Anfangszustand.}
{Der Benutzer drückt \enquote{\textbackslash}.}
{Im Editor erscheint ein $\lambda$. Kein \enquote{\textbackslash} wird hinzugefügt.}

\teststep {Zustand aus vorherigem Schritt.}
{Der Benutzer gibt $\lambda x.x$ in das Eingabefeld ein und drückt auf \enquote{Run}.}
...
{Das Programm hört auf $\beta$-Redutkionen zu berechnen. Es ist allerdings möglich auf den \enquote{Continue} Button zu drücken um weiter zu Rechnen}

%% TODO add click on continue button

\test{Einfaches Eingabe und Run Szenario.}{tst:simple-input-run}
\tests{fnc:export-latex}
\tests{fnc:comments}
\tests{fnc:bracket} % TODO
\tests{fnc:input}
\tests{fnc:editor}
\tests{fnc:run}
...
{Im Eingabefeld steht $(\lambda x.x)$. Es wird $\lambda x.x$ ausgegeben.} % TODO vielleicht hieran noch Stop/Pause und ausgrauungen testen?
