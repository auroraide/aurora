...
\criteriumOptional{Einzelnen Schritt auswerten}{crt:single-step-eval}
...
\functionality{Terminate}{fnc:stop}
\fulfills{crt:stop}
...
\fulfills{crt:pause}
Der Benutzer kann auf den \enquote{Pause}-Button drücken um die laufende Berechnung zu unterbrechen und sich die bisherigen Schritte anzeigen zu lassen. Er kann allerdings, im Gegensatz zum \enquote {Terminate}-Button, die Berechnung weiterlaufen lassen.
...
Durch Eingabe von \enquote {\textbackslash} wird ein \enquote {$\lambda$} ausgegeben.

\functionality{Run-Shortcut}{fnc:run-shortcut}
Drücken von Shift \enquote {+} $\hookleftarrow$ ist äquivalent zum Drücken des \enquote {Run}-Buttons.

\functionality{Terminate-Shortcut}{fnc:terminate-shortcut}
Drücken von Shift \enquote {+} t ist äquivalent zum Drücken des \enquote {Terminate}-Buttons.

\functionality{Pause-Shortcut}{fnc:pause-shortcut}
Drücken von Shift \enquote {+} p ist äquivalent zum Drücken des \enquote {Paues}-Buttons.

\functionality{Schritt}{fnc:step-shortcut}
Drücken von Shift \enquote {+} n ist äquivalent zum Drücken des \enquote {Schritt}-Buttons.
...
\nonFunctionality{Lambda Eingabe}{nfc:lambda-input}
...
\test{Einfaches Eingabe- und Runzenario.}{tst:simple-input-run}
\tests{fnc:comments}
\tests{fnc:bracket}
\tests{fnc:input}
\tests{fnc:output}
...
% TODO \tests error printing
\teststep{Anfangszustand nach dem öffnen von $\lambda$urora.}
{Der Benutzer gibt \texttt{(} ein.}
{Im Eingabefeld steht jetzt $()$. Die Klammer zu wurde verfollständigt.}

\teststep {Zustand vom letzten Schritt.}
...
\test{Run, Pause, Step}{tst:run-pause-stop}
\tests{fnc:input}
%\tests{fnc:ln}
\tests{fnc:editor}
...
\teststep{Zustand vom vorherigen Schritt. Die Berechnung läuft.}
...
\teststep{Zustand aus dem vorherigem Schritt. } % \tests{fnc:no-edit-while-running}
...
\test{Rückersetzung}{tst:resubstitution}
% TODO tests
\teststep{Anfangszustand.}
...
%{Das Programm gibt $$\lambda s.\lambda z.(\,s\,(\,s\,(\,s\,(s\,z)))$$ im Ausgabefeld aus. (Beziehungsweise $4$, wenn das Wunschkriterium Standardbibliothek vollständig implementiert ist).}




\test{Sprachenauswahl}{tst:settings}
...
\teststep{Es steht eine gültige Eingabe im Eingabefeld.
...
\teststep {Nach dem vorherigen Test wurde noch fünf mal aud den \enquote {drei-Punkte}-Button gedrückt, es sind also 90 Zwischenergebnisse sichtbar.}
  ...
  \begin{description}
	...
	\begin{itemize}
		...
		\item \textbf{Editor / IDE}
            \newline Freie Wahl durch den Entwickler, benutzt werden unter anderem:
		\begin{itemize}
			\item Eclipse
            \item IntelliJ
            \item Atom
