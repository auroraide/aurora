...
\test{Einzelauswertung}{tst:single-eval}
\tests{fnc:step}
\teststep{Das Programm befindet sich im Default-State.}
{Der Benutzer gibt \enquote {\$plus 2 2} ein und drückt auf \enquote {Step}.}
{Im Schrittfenster wird ausgegeben:
    \newline ($\lambda$n.$\lambda$m.$\lambda$s.$\lambda$z.n s (m s z)) ($\lambda$s.$\lambda$z.s(s z)) ($\lambda$s.$\lambda$z.s(s z))}


\test{Eingabe und korrektes Ergebnis}{tst:input-and-result}
\tests{fnc:shortcut}
\tests{fnc:input}
\tests{fnc:ln}
\tests{fnc:editor}
...
{Das Programm überspringt den Lambdaterm in der Kommentarzeile und berechenet nur den aus der unteren Zeile. Das Programm gibt y aus.
