...
\subsection{Eingabe}
\functionality{Eingabefeld}{fnc:input}\fulfills{crt:text-editor}\fulfills{crt:not-editable}
...
\functionality{Eingabefeld bearbeiten}{fnc:editor}\fulfills{crt:text-editor}\fulfills{crt:not-editable}
...
\functionality{Shortcuts}{fnc:shortcut}\fulfills{crt:shortcuts}
Der Benutzer kann Tastenkürzel benutzen um das Programm mit Einschränkungen zu bedienen. 

\functionality{Highlightning Lambdasymbol und Punkte}{fnc:highlightstatic}\fulfills{crt:syntax-highlighting}
...
\functionality{Auswertungsstrategien}{fnc:eval-order-options}\fulfills{crt:opt-eval-strategy}\fulfills{crt:own-eval-strategy}
...
\functionality{Der Benutzer kann selber Redex auswählen, der als nächstes reduziert wird}{fnc:choosingredex}\fulfills{crt:own-eval-strategy}
...
\functionality{Highlighting von Funktsionsnamen und Klammern}{fnc:highonbrackets}\fulfills{crt:syntax-highlighting}
...
\functionality{Line Numbers}{fnc:ln}\fulfills{crt:linenumbers}
...
\functionality{Tutorial zur Web App}{fnc:tutorial}\fulfills{crt:tutorial}
...
\functionality{Nach Wunsch der User werden die $\beta$-Reduktionsschritte angezeigt}{fnc:stepbystep}\fulfills{crt:intermediate-step-output}
...
\functionality{Sprache ist auswählbar}{fnc:language-selection}\fulfills{crt:opt-language}
...
\functionality{Stop-Button}{fnc:stop-exec} \fulfills{crt:stop-feature}
...
\functionality{Offensichtliche Schleifen erkennung}{fnc:loop-test}\fulfills{crt:loop-detection}
...
\functionality{Pause-Button} {fnc:pausebut} \fulfills{crt:pausebutton}
...
\functionality{Nach neuer Tab}{fnc:to-new-tab}\fulfills{crt:to-new-tab}
...
\functionality{Share-Button}{fnc:share-button}\fulfills{crt:share-feature}
...
\functionality{Berechnung in LaTeX syntax umwandeln und zum Copy-to-Clipboard hinzufügen}{fnc:latexshare}\fulfills{crt:latex-export}
...
\nonFunctionality{Unterstützde Sprachen}{nfc:supported-languages}
